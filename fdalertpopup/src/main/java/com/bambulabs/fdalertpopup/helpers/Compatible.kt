package com.bambulabs.fdalertpopup.helpers
...
private val gradientState =
    resolveGradientState()
...
fun setUseLevelForShape(drawable: GradientDrawable, value: Boolean) {
    try {
        val useLevelForShape = resolveField(
            gradientState,
            "mUseLevelForShape"
        )
...
fun setOrientation(drawable: GradientDrawable, value: GradientDrawable.Orientation) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
        ...
        try {
            val orientation = resolveField(
                gradientState,
                "mOrientation"
            )
            orientation.set(drawable.constantState, value)
            val rectIdDirty = resolveField(
...
fun setGradientRadius(drawable: GradientDrawable, value: Float) {
    try {
        val gradientRadius = resolveField(
            gradientState,
            "mGradientRadius"
        )
...
fun setDrawable(rotateDrawable: RotateDrawable, drawable: Drawable) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
        ...
        try {
            val drawableField = resolveField(
                rotateState,
                "mDrawable"
            )
            val stateField = resolveField(
                RotateDrawable::class.java,
                "mState"
            )
...
fun setPivotX(rotateDrawable: RotateDrawable, value: Float) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
        ...
        try {
            val pivotXField = resolveField(
                rotateState,
                "mPivotX"
            )
            pivotXField.setFloat(rotateDrawable.constantState, value)
            val pivotXRelField = resolveField(
                rotateState,
                "mPivotXRel"
            )
...
fun setPivotY(rotateDrawable: RotateDrawable, value: Float) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
        ...
        try {
            val pivotYField = resolveField(
                rotateState,
                "mPivotY"
            )
            pivotYField.setFloat(rotateDrawable.constantState, value)
            val pivotYRelField = resolveField(
                rotateState,
                "mPivotYRel"
            )
...
fun setToDegrees(rotateDrawable: RotateDrawable, value: Float) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
        ...
        try {
            val toDegreesField = resolveField(
                rotateState,
                "mToDegrees"
            )
